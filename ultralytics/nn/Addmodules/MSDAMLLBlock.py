...
__all__ = ['MSDAMLLABlock','C3k2_MSDAMLLABlock','C3k2_MSDAMLLABlock1', 'C3k2_MSDAMLLABlock2']
...
class MSDAMLLABlock(nn.Module):
    def __init__(self, c, num_heads_mdla=8, num_heads_mlla=4, shortcut=True):
        super().__init__()
...
class C3kMLLABlock(C3):
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5, k=3):
        super().__init__(c1, c2, n, shortcut, g, e)
        c_ = int(c2 * e)
        self.m = nn.Sequential(*(MSDAMLLABlock(c_) for _ in range(n)))


class C3k2_MSDAMLLABlock(C3):
    """C3k3变体，使用MSDAMLLABlock替换Bottleneck，重复3次。"""
    def __init__(self, c1, c2, n=3, shortcut=True, g=1, e=0.5):  # n=3模拟C3k3
        super().__init__(c1, c2, n, shortcut, g, e)
        c_ = int(c2 * e)  # 中间通道
        self.m = nn.Sequential(*(MSDAMLLABlock(c_) for _ in range(n)))  # 替换为MSDAMLLABlock x3

    def forward(self, x):
        # 继承C3的forward：cv1 + cv2 + 串联m
        return super().forward(x)


class C3k2_MSDAMLLABlock1(C2f):
    def __init__(self, c1, c2, n=1, c3k=False, e=0.5, g=1, shortcut=True):
        super().__init__(c1, c2, n, shortcut, g, e)
        self.m = nn.ModuleList(C3k(self.c, self.c, 2, shortcut, g) if c3k else MSDAMLLABlock(self.c) for _ in range(n))

class C3k2_MSDAMLLABlock2(C2f):
    def __init__(self, c1, c2, n=1, c3k=False, e=0.5, g=1, shortcut=True):
        super().__init__(c1, c2, n, shortcut, g, e)
        self.m = nn.ModuleList(C3kMLLABlock(self.c, self.c, 2, shortcut, g) if c3k else Bottleneck(self.c, self.c, shortcut, g) for _ in range(n))
