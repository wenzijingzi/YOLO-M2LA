...
__all__ = ['C3k2_MSDAMLLABlock1', 'C3k2_MSDAMLLABlock2']
...
class MSDAMLLABlock(nn.Module):
    ...
    def forward(self, x):
        # 先应用MultiDilatelocalAttention (MDLA)
        residual = x
        x = self.mdla(x)
        if self.shortcut:
            x = x + residual
...
class C3k2_MSDAMLLABlock1(C2f):
    ...
    def __init__(self, c1, c2, n=1, c3k=False, e=0.5, g=1, shortcut=True):
        ...
        self.m = nn.ModuleList(
            C3k(self.c, self.c, 2, shortcut, g) if c3k else MSDAMLLABlock(self.c,) for _ in range(n)
        )
