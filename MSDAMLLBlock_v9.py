...
__all__ = ['MSDAMLLABlock', 'ELAN1', 'RepNCSPELAN4', 'RepNCSPELAN4_MSDAMLLA','RepNCSPELAN4_MSDAMLLA1']
...
class MLLABlock(nn.Module):
    def __init__(self, dim, num_heads=4, mlp_ratio=4., qkv_bias=True, drop=0., drop_path=0.,
        ...
        super().__init__()
        self.dim = dim
        self.num_heads = num_heads  # 固定使用传入num_heads，避免重新计算
    ...
    def forward(self, x):
        B, C, H, W = x.shape
        assert H * W * C == x.numel() / B, "Input shape mismatch"  # 添加形状验证
...
class MultiDilatelocalAttention(nn.Module):
    def __init__(self, dim, num_heads=4, qkv_bias=True, qk_scale=None,  # 减少heads
    ...
    def forward(self, x):
        ...
        for i in range(self.num_dilation):
            y1[i] = self.dilate_attention[i](qkv[i][0], qkv[i][1], qkv[i][2])  # 形状: (B, d//num_dilation, H, W)
...
# 新串联块：MSDAMLLABlock (MDLA + MLLA)
class MSDAMLLABlock(nn.Module):
    def __init__(self, c, num_heads_mdla=4, num_heads_mlla=2, shortcut=True):  # 减少heads控计算
        super().__init__()
...
class RepNCSPELAN4_MSDAMLLA(RepNCSPELAN4):
    """RepNCSPELAN4变体，使用MSDAMLLABlock替换RepCSP。"""
    def __init__(self, c1, c2, c3, c4, n=1):
        super().__init__(c1, c2, c3, c4, n)
        self.cv2 = nn.Sequential(MSDAMLLABlock(c4), Conv(c4, c4, 3, 1))  # 替换RepCSP为MSDAMLLABlock
        self.cv3 = nn.Sequential(MSDAMLLABlock(c4), Conv(c4, c4, 3, 1))  # 同上，重复n=1模拟

class RepNCSPELAN4_MSDAMLLA1(nn.Module):
    """RepNCSPELAN4变体，使用MSDAMLLABlock替换原分支路径，实现注意力机制增强。"""
    def __init__(self, c1, c2, c3, c4, n=1):
        super().__init__()
        self.c = c3 // 2
        self.cv1 = Conv(c1, c3, 1, 1)
        # 使用MSDAMLLABlock替换原RepCSP(c3//2 -> c4)，并后接Conv(3,1)
        self.cv2 = nn.Sequential(MSDAMLLABlock(c3 // 2), Conv(c3 // 2, c4, 3, 1))
        # 使用MSDAMLLABlock替换原RepCSP(c4 -> c4)，并后接Conv(3,1)
        self.cv3 = nn.Sequential(MSDAMLLABlock(c4), Conv(c4, c4, 3, 1))
        self.cv4 = Conv(c3 + (2 * c4), c2, 1, 1)

    def forward(self, x):
        """Forward pass through RepNCSPELAN4_MSDAMLLA1 layer."""
        y = list(self.cv1(x).chunk(2, 1))
        y.extend((m(y[-1])) for m in [self.cv2, self.cv3])
        return self.cv4(torch.cat(y, 1))
...
if __name__ == "__main__":
    # 测试RepNCSPELAN4_MSDAMLLA（修复原错误：C3kMLLABlock未定义）
    image_size = (1, 64, 224, 224)  # 测试方形
    image = torch.rand(*image_size)
    model = RepNCSPELAN4_MSDAMLLA(64, 128, 64, 32, n=1)  # 示例参数
    out = model(image)
    print(out.size())  # 应为(1,128,224,224)

    # 测试矩形
    image_size_rect = (1, 64, 128, 256)
    image_rect = torch.rand(*image_size_rect)
    out_rect = model(image_rect)
    print(out_rect.size())  # 应为(1,128,128,256)